...
# input_data = 'A(1x5)BC'
# input_data = '(3x3)XYZ'
# input_data = 'A(2x2)BCD(2x2)EFG'
# input_data = '(6x1)(1x3)A'
...
next_match = marker_regex.search(input_data, current_pos)

while next_match:
    length_to_repeat = int(next_match.group(1))
    repetitions = int(next_match.group(2))

    str_to_repeat_start_idx = next_match.span()[1]
    str_to_repeat_end_idx = str_to_repeat_start_idx + length_to_repeat

    str_to_repeat = input_data[str_to_repeat_start_idx:str_to_repeat_end_idx]
    str_to_replace = next_match.group(0) + str_to_repeat
    replacement = str_to_repeat * repetitions

    input_data = input_data.replace(str_to_replace, replacement, 1)
    #current_pos = next_match.span()[0] + len(replacement)
